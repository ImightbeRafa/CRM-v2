...
- Prove (post-#74 rebase): focused display-name + accounts + identity-refresh tests + `npm run test:chat-harden` + `npm run test:soft-ai-agent`. Lint existing warnings only.
