...
| **Arc 2 plan (PLAN GO 2026-09-21, no code yet)** | [`docs/plans/betsy-agent-layer-arc2-fable-2026-09-21.md`](../plans/betsy-agent-layer-arc2-fable-2026-09-21.md) — smarter Forge WA agent (payment classifier, brand facts, shortcuts, images, Canales, multi-turn Probar), SINPE payment queue in `/chats` + staff notify outbox, post-approve order prompts on existing EA/RA paths, horario + reply delay. PRs `AL2-A1` → `AL2-A2` → `AL2-B` → `AL2-C` → `AL2-D`; SQL `029`/`030` gated. Each phase needs its own Rafael GO. |
