...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const employee = employeeRow(rows[0]);
    await audit(getRequestActorId(req.headers), 'employee_created', employee.id, {
      displayName,
      hourlyRateCrc,
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    ...
    const employee = employeeRow(rows[0]);
    await audit(getRequestActorId(req.headers), regenerateCode ? 'employee_code_regenerated' : 'employee_updated', employee.id, {
      displayName,
      active,
      hourlyRateCrc,
    ...
    return NextResponse.json({ employee, code: generated?.code ?? null });
  } catch (error) {
