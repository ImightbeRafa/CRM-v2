...
// Comunicación panels stay mounted once opened (see keep-alive block): an in-flight
// Embedded Signup / IG popup and unsaved agent drafts survive tab switches.
const panelSkeleton = () => <AuroraListSkeleton rows={5} label="Cargando panel" />
const SocialPanelPage = dynamic(() => import('./social/page'), { ssr: false, loading: panelSkeleton })
const AgentesPanelPage = dynamic(() => import('./agentes/page'), { ssr: false, loading: panelSkeleton })
const IntegrationsPanel = dynamic(
  () => import('@/components/aurora/config/panels/IntegrationsPanel').then((m) => m.IntegrationsPanel),
...
function ConfigPageInner() {
  ...
  const [activeTab, setActiveTab] = useState<string>(() => {
    return normalizeTabParam(searchParams?.get('tab'))
  })
  // Comunicación panels opened this session (kept mounted, hidden when inactive).
  ...
  // The URL is the source of truth (Back/Forward walk the tabs). Non-canonical `?tab=`
  // values are redirected by the server page; this is the client-side safety net.
  useEffect(() => {
    if (!mounted) return
    const { href, changed } = normalizeConfigSearch(searchParams?.toString() ?? '')
    if (changed) {
  ...
  useEffect(() => {
    if (!(KEEP_ALIVE_TABS as readonly string[]).includes(activeTab)) return
  ...
  // Optimistic highlight only: sub-nav Links push the URL, which then drives `activeTab`.
  const selectTab = (tabId: string) => {
    setActiveTab(tabId)
  }
  // Hub cards navigate programmatically (push, so Back returns to the hub).
  const openTab = (tabId: string) => {
    setActiveTab(tabId)
    router.push(tabId === CONFIG_HUB_TAB ? '/config' : `/config?tab=${encodeURIComponent(tabId)}`, { scroll: false })
  }
  ...
  return (
    <ConfigShell activeTab={activeTab} onSelectTab={selectTab}>
        <div className="space-y-6">
         <Suspense fallback={<AuroraListSkeleton rows={5} label="Cargando panel" />}>
          {activeTab === CONFIG_HUB_TAB && <ConfigHub onOpenTab={openTab} />}
          ...
          {activeTab === 'import' && (
            <>
              <ConfigPanelHeader title="Importar" subtitle="Traé tus productos desde un archivo Excel" />
              <ExcelImporter />
            </>
          )}
          ...
          {/* Comunicación panels: mounted on first open, then kept alive (hidden) so popups and drafts survive tab switches. */}
          {visited.has('social') && (
          ...
          {visited.has('agentes') && (
            <div hidden={activeTab !== 'agentes'} className={PAGE_PANEL_BLEED}>
              <AgentesPanelPage />
            </div>
          )}
