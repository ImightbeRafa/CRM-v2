...
export function PaginatedClientManagement({ onUnavailable }: { onUnavailable: () => void }) {
  ...
  if (dataTenantKey !== tenantKey || (loading && clients.length === 0)) return <div><ConfigPanelHeader title="Clientes" subtitle="Tus clientes frecuentes y sus datos de contacto" /><AuroraListSkeleton rows={5} label="Cargando clientes" /></div>;
  return <div className="space-y-6">
    <ConfigPanelHeader className="!mb-0" title="Clientes" subtitle="Tus clientes frecuentes y sus datos de contacto" actions={<><Button variant="outline" onClick={() => void sync()}><RefreshCw className="h-4 w-4 mr-2" />Sincronizar</Button><Button variant="outline" onClick={exportClients}><Download className="h-4 w-4 mr-2" />Exportar</Button><Button className="bg-[#5B3FE0] text-white hover:bg-[#4A32C4]" onClick={() => openForm()}><Plus className="h-4 w-4 mr-2" />Agregar cliente</Button></>} />
