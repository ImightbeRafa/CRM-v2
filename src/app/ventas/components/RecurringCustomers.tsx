...
const RecurringCustomers: React.FC<RecurringCustomersProps> = ({
  ...
  return (
    <div className="space-y-3 mb-4 p-4 bg-purple-50 rounded-lg border border-purple-200">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Users className="w-5 h-5 text-purple-600" />
          <h4 className="font-semibold text-purple-800">Clientes Recurrentes</h4>
        </div>
        <Button
          type="button"
      ...
      <div className="relative">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 border border-purple-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 text-sm"
          />
        </div>

        {/* Dropdown Results */}
        {showDropdown && filteredClients.length > 0 && (
          <div className="absolute z-50 w-full mt-1 bg-white border border-purple-200 rounded-lg shadow-lg max-h-64 overflow-y-auto">
            {filteredClients.slice(0, 10).map((client) => (
              <button
                key={client.id}
                type="button"
                onClick={() => handleSelectClient(client)}
                className="w-full text-left p-3 hover:bg-purple-50 border-b border-gray-100 transition-colors"
              >
                <div className="flex items-center justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            {topClients.map((client) => (
              <button
                ...
                type="button"
                onClick={() => handleSelectClient(client)}
                className="inline-flex items-center gap-1 px-3 py-1.5 bg-white border border-purple-200 rounded-full hover:bg-purple-100 transition-colors text-sm"
              >
                {client.isFavorite && <Star className="w-3 h-3 text-yellow-500 fill-yellow-500" />}
