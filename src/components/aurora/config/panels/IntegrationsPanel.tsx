...
export function IntegrationsPanel() {
  ...
  if (loading) {
    return <AuroraListSkeleton rows={4} label="Cargando integraciones" />;
  }

  if (loadError && apiKeys.length === 0) {
    return (
      <AuroraErrorState
  ...
  return (
    <div className="space-y-6">
      <ConfigPanelHeader
        ...
        actions={
          <Button onClick={() => setShowNewKeyDialog(true)} className="flex items-center gap-2 bg-[#5B3FE0] text-white hover:bg-[#4A32C4]">
        ...
        <CardContent>
          {apiKeys.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p>Todavía no hay API keys</p>
              <p className="text-sm">Creá una para conectar tu sitio web</p>
            </div>
          ) : (
